      ...
      req.token = tokenBearer;
       ...
       jwt.verify(tokenBearer, SECRET_KEY, async (err, decoded)=> {
          ...
          try {
            ...
            if (!admin) {
              return res.status(403).send({ auth: false, message: 'You dont have access to this resource.' })
            }
            req.user = decoded
            next();
          } catch (err) {
            return res.status(500).send({ auth: false, message: 'An error occurred while trying to authenticate given token.' });
          }
